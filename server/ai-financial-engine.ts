...
export class AIFinancialEngine {
  ...
  /**
   * Get AI client with Grok API
   */
  private static async getAIClient(): Promise<OpenAI | null> {
    try {
      const apiKey = await AdminSettingsManager.getSetting('XAI_API_KEY');
      if (!apiKey) {
        console.warn('XAI_API_KEY not found in admin settings');
        return null;
      }
      ...
      console.error('Error initializing xAI client:', error);
      return null;
    }
  }
  ...
  /**
   * Generate portfolio optimization recommendations with Grok AI
   */
  private static async generatePortfolioOptimizationRecommendations(
    ...
    // Use Grok AI for enhanced portfolio optimization insights
    ...
    if (aiClient) {
      try {
        const portfolioData = {
          ...
          performanceVsBenchmark: analysis.performanceVsBenchmark,
          userProfile: {
        ...
        const aiResponse = await aiClient.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are an expert financial advisor providing personalized investment recommendations. Analyze the portfolio data and provide specific, actionable recommendations in JSON format."
            },
            {
              role: "user",
              content: `Analyze this portfolio and provide 1-2 specific optimization recommendations: ${JSON.stringify(portfolioData)}`
            }
          ],
        ...
        const aiRecommendation = JSON.parse(aiResponse.choices[0].message.content || '{}');
        
        if (aiRecommendation.recommendations) {
          aiRecommendation.recommendations.forEach((rec: any, index: number) => {
            recommendations.push({
              id: `ai-portfolio-${Date.now()}-${index}`,
              type: 'portfolio_rebalance',
              title: rec.title || 'AI Portfolio Optimization',
              description: rec.description || 'AI-powered portfolio optimization recommendation',
              priority: rec.priority || 'medium',
              expectedReturn: rec.expectedReturn || 3.5,
              riskLevel: rec.riskLevel || 3,
              timeframe: rec.timeframe || '1-2 months',
              actionItems: rec.actionItems || ['Follow AI recommendation'],
              reasoning: rec.reasoning || 'Generated by Grok AI analysis',
              confidence: 0.9,
              potentialGains: analysis.currentValue * 0.12,
              potentialLosses: analysis.currentValue * 0.04,
              marketFactors: ['AI-powered market analysis', 'Advanced portfolio optimization']
            });
          });
        }
      } catch (error) {
        console.error('Error generating AI portfolio recommendations:', error);
      }
    }
